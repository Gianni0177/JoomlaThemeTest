...
$completewebsite = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
            		...
            		if(!empty($extractedHTags['h'.($upperlevel+1)]) ) {
                    	...
                    	//echo '<ul>';
                    		foreach($extractedHTags['h'.($upperlevel+1)] as $extractedHTag) {
                            	echo '<li class="nav-item '. ($firstt?'active':'') .'"><a class="nav-link '. ($firstt?'active':'') .'" href="#' . strClean($extractedHTag) . '"><span>' . $extractedHTag . '</span></a></li>';
                              $firstt=false;
                            }
                    	//echo '</ul>';
                    } else {
...
<?xml encoding="utf-8" ?><html><body>', "", $contenttext);
	...
	for (; $index <= count($ctpieces)-1; $index++) {
        ...
        <article id="<?php echo strClean($extractedHTags['h'.($upperlevel+1)][$hindex]) ?>" class="it-page-section anchor-offset">
          ...
          <?php echo '<h'.($upperlevel+1).'>'.$extractedHTags['h'.($upperlevel+1)][$hindex].'</h'.($upperlevel+1).'>' ?>
