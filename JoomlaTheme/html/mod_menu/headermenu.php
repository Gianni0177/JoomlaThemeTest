...
$attributes          = [];
$attributes['class'] = 'navbar-nav ' . $class_sfx;
...
<?php foreach ($list as $i => &$item)
{
	...
	switch ($item->type) :
		...
		case 'heading':
		case 'url':
			require ModuleHelper::getLayoutPath('mod_menu', 'headermenu_' . $item->type);
			break;

		default:
			require ModuleHelper::getLayoutPath('mod_menu', 'headermenu_url');
	endswitch;

	switch (true) :
		// The next item is deeper.
		case $showAll && $item->deeper:
			echo '<div class="dropdown-menu" role="region" aria-labelledby="nav_item_link_' . $item->id . '"><div class="link-list-wrapper"><ul class="link-list">';
			break;
