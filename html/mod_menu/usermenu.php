...
$attributes          = [];
$attributes['class'] = 'link-list ' . $class_sfx;
...
<?php foreach ($list as $i => &$item)
{
	...
	$class      = [];
	$class[]    = 'link-list-item item-' . $item->id . ' level-' . ($item->level - $start + 1);
	...
	switch ($item->type) :
		...
		case 'heading':
		case 'url':
			require ModuleHelper::getLayoutPath('mod_menu', 'usermenu_' . $item->type);
			break;

		default:
			require ModuleHelper::getLayoutPath('mod_menu', 'usermenu_url');
	endswitch;
